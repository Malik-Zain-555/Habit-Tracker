...
router.get('/', auth, async (req, res) => {
    try {
        const db = req.db;
        const habitsSnapshot = await db.collection('habits').where('userId', '==', req.user.userId).get();
...
router.put('/:id', auth, async (req, res) => {
    try {
        ...
        if (doc.data().userId !== req.user.userId) return res.status(403).json({ error: 'Unauthorized' });
...
router.post('/:id/complete', auth, async (req, res) => {
    try {
        ...
        const lastCompleted = doc.data().lastCompleted ? new Date(doc.data().lastCompleted).setHours(0,0,0,0) : null;
        ...
        res.json({ _id: req.params.id, ...doc.data(), currentStreak: newStreak, lastCompleted: now.toISOString(), completionDates });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
